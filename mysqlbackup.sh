...
innobackupex --user ${user} --password ${password}  --no-timestamp ${full_dir}/full-`date +%Y%m%d%H%M%S`
}
...
if [ ${nozore} -eq 0 ];then
   innobackupex --user ${user} --password ${password} --no-timestamp --incremental ${increment_dir}/increment-`date +%Y%m%d%H%M%S` --incremental-basedir=${full_dir}/$last1 
   #用完整备份做第一次增量备份
else 
   innobackupex --user ${user} --password ${password}  --no-timestamp --incremental ${increment_dir}/increment-`date +%Y%m%d%H%M%S` --incremental-basedir=${increment_dir}/$last2
   #用上次增量备份做增量备份 
fi
}
...
for i in ${list}
do
    innobackupex --user ${user} --password ${password}  --apply-log --redo-only ${increment_dir}/$i  #对最近7天内的增量备份作apply-log redo-only操作
done
...
while :
do
  echo -e "\033[31m 请小心操作，数据谨防数据丢失! \033[0m"
